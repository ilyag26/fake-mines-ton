...
function App() {
  ...
  const countActiveCells = useRef(0);
  const [finalCoef, setFinalCoef] = useState(0);
  const [winAmout, setWinAmout] = useState(0);
  const [betDollarConvert, setBetDollarConvert] = useState(0);
  ...
  const clickCell = (id) => {
    countActiveCells.current++;
    console.log(countActiveCells.current)
    if (buttonActive) {
      setCellStatus(prev => ({
        ...prev,
        [id]: true
  ...
  useEffect(() => {
    const newVal = bet + bet * getRandomFloat(0.0001, 0.0004);
    setBetDollarConvert(newVal);
  }, [bet]);

  const finishAll = () => {
    if (countActiveCells.current != 25 - traps) {
      const winCoef = coefMultiplicator[traps][countActiveCells.current];
      setFinalCoef(winCoef);
      setBalance(prev => prev + bet * winCoef);
      setWinAmout(bet * winCoef);
    } else {
      const winCoef = getLastElement();
      setFinalCoef(winCoef);
      setBalance(prev => prev + bet * winCoef);
      setWinAmout(bet * winCoef);
    }
  ...
  const getLastElement = () => {
    const keys = Object.keys(coefMultiplicator[traps]);
    const lastKey = keys[keys.length - 1];
    const lastVal = coefMultiplicator[traps][lastKey];
    return lastVal;
  }
  ...
  const btnActive = () => {
    ...
    if (buttonActive == 2) {
      ...
      setFinalCoef(0);
      setWinAmout(0);
      countActiveCells.current = 0;
    }

  }

  const fairnessSwitcher = () => {
    if (showFairness)
  ...
  const showBalanseSwitcher = () => {
    if (!showBalanceChanger) {
      setShowBalanceChanger(prev => !prev);
    } else {
      setShowBalanceChanger(false);
    }
  }
  ...
  return (
    <>
      ...
      {showBalanceChanger && (
        <>
          <div className="honesty-modal fade-in">
            <div className="container-honesty">
              <input type="number" value={balance} onChange={handleChange} placeholder="Enter new balance" />
              <button onClick={showBalanseSwitcher}>Close</button>
            </div>
          </div>
        </>
      )}
      ...
      {showFairness && (
        <>
          <div className="honesty-modal fade-in">
            <div className="container-honesty">
              <div className="close" onClick={fairnessSwitcher}></div>
              <section className="white none-padding">
                <h3 className="section-title">Fairness</h3>
              </section>
              <section className="white none-padding">
                <div className="row">
                  <div className="mb-3 bet-container">
                    <div className="container-switch">
                      <div className="level  active-level">Seeds
                      </div>
                      <div className="level ">Verification</div>
                    </div>
                  </div>
                </div>
              </section>
              ...
              <section className="white none-padding">
                ...
                <div className="mb-3 bet-container">
                  <label for="exampleInputEmail1" class="form-label">New Client Seed</label>
                  <input id="bet" class="ember-text-field ember-view form-control form-control-updated" placeholder="" defaultValue={hashServer} type="text" />
                  <button style={{ right: 0 }} class="btn btn-primary change-btn bet-btn" type="button">Change</button>
                </div>
                <div className="col col-12 mb-3 bet-container">
                  <label className="form-label">Next Server Seed <span className="hash-label">(hashed)</span></label>
                  <div className="btn-display-input-container btn-copy">
                    <button className="form-control display-input" id="copyInputNextSeedKey" type="button">
                      <div className="input-text">
                        1b0b5fee477a4cc42d82a4621c546795411c2ba707e6dc7ac7c718d638508f15
                      </div>
                    </button>
                    <button className="copy-btn copy-btn-special-3" data-bs-placement="top" title="Address copied!" data-bs-toggle="tooltip" type="button" data-ember-action="" data-ember-action-101="101"></button>
                  </div>
                </div>
              </section>
            </div>
          ...
          <section id="header">
            <div className="wrapper justify-content-between">
              ...
              <div className="dropdown dropdown-wallets">
                ...
                <a className="openCurrency" data-ember-action="" data-ember-action-6="6" onClick={showBalanseSwitcher}></a>
              ...
              {winWindow && (
                <div className="winner-block">
                  ...
                  <div className="winner-container">
                    <div className="winner-title">{finalCoef}x</div>
                    <div className="line-container">
                      <div className="winner-line"></div>
                    </div>
                    <div className="winner-text">{winAmout.toFixed(7)}<span className="crypto-logo">💎</span></div>
                  </div>
                </div>
              )}
              <div className="game ">
            ...
            <section className="white none-padding">
              <div className="row">
                <div className="mb-3 bet-container special-btn-bet-container">
                  ...
                  <span className="dollars">{betDollarConvert.toFixed(2)} $</span>
                  <div className="input-container">
                    <div className="input-logo-container">
                      <input id="bet" className="ember-text-field ember-view form-control" placeholder="0.0000000" onChange={handleInputBet} type="text"></input>
              ...
              <div className="footer-game">
                <div className="row">
                  ...
                  <div className="col col">
                    <a className="honesty-btn" data-ember-action="" data-ember-action-45="45" onClick={fairnessSwitcher}>Fairness</a>
                  </div>
                </div>
              </div>
            </section>
